/**
 * LÝ DO GIỮ LẠI FILE NÀY:
 * Theo yêu cầu của hệ thống, file này được giữ lại làm tham khảo về logic cũ.
 * Hệ thống đã chuyển sang sử dụng azureAiUtil.ts (Azure OpenAI).
 * Toàn bộ logic bên dưới sử dụng Google Gemini và đã được comment lại.
 */
...
//     - **Output Format:** Return ONLY a raw JSON object. Do not use Markdown code blocks (no \`\`\`json). Do not add any conversational text.
// 
//     ### GRAPH REQUIREMENTS:
...
//           "id": "node_1",
//           "label": "Concept Name",
//           "category": "core|algorithms|systems|other",
//           "importance": 3,
...
//   try {
//     const response = await ai.models.generateContent({
//       model: "models/gemini-2.5-flash", // Use standard flash for larger context window
//       contents: prompt,
//       config: {
...
//     
//     // Validate with Zod
//     const validatedData = KnowledgeGraphSchema.parse(parsedJson);
//     
//     // Ensure edge integrity
//     const nodeIds = new Set(validatedData.nodes.map(n => n.id));
//     validatedData.edges = validatedData.edges.filter(
//       edge => nodeIds.has(edge.from) && nodeIds.has(edge.to)
//     );
// 
//     return validatedData;
//   } catch (error) {
//     console.error("Gemini API error generating knowledge graph:", error);
