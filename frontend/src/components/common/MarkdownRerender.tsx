...
type MarkdownElementProps = any;

type MarkdownCodeProps = any;
    ...
    <div className="text-slate-700 leading-7 text-[15px]">
      <ReactMarkdown
        ...
        components={{
          ...
          thead: ({ node, ...props }: MarkdownElementProps) => (
            <thead className="bg-slate-50" {...props} />
          ),
          tbody: ({ node, ...props }: MarkdownElementProps) => (
            <tbody className="divide-y divide-slate-200 bg-white" {...props} />
          ),
          tr: ({ node, ...props }: MarkdownElementProps) => (
            <tr className="transition-colors hover:bg-slate-50/50" {...props} />
          ),
          th: ({ node, ...props }: MarkdownElementProps) => (
            <th
              className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider"
              {...props}
            />
          ),
          td: ({ node, ...props }: MarkdownElementProps) => (
            <td
          ...
          // --- Images ---
          img: ({ node, ...props }: MarkdownElementProps) => (
