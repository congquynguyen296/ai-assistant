...
const QuizManager = ({ documentId }: QuizManagerProps) => {
  ...
  const fetchQuizzes = async (documentId: string) => {
    setLoading(true);

    try {
      const response = await quizService.getQuizzesForDocument(documentId);
      setQuizzes((response?.data as any)?.quizzes || response?.data || []);
    } catch (error) {
  ...
  const handleGenerateQuiz = async ({ count, title, requirements }: { count: number; title: string; requirements: string }) => {
    setGenerating(true);
