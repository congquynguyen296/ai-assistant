...
interface JobInterviewSetupProps {
  onChange: (params: { documentIds?: string[]; customText?: string }) => void;
}

export default function JobInterviewSetup({ onChange }: JobInterviewSetupProps) {
  ...
  }, [cvMode, jdMode, cvDocId, jdDocId, cvText, jdText, onChange]);

  const renderInputMode = (
    label: string,
    mode: 'select' | 'paste',
    setMode: (m: 'select' | 'paste') => void,
    docId: string,
      ...
      <div className="space-y-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row bg-slate-100 p-1 rounded-xl gap-1 sm:gap-0">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-lg transition-all ${mode === 'select' ? 'bg-white shadow-sm text-emerald-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
            <FileText className="w-4 h-4" /> Chọn tài liệu
          </button>
          <button
            onClick={() => setMode('paste')}
            className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-lg transition-all ${mode === 'paste' ? 'bg-white shadow-sm text-emerald-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
        ...
        <div className="mt-4">
          ...
          {mode === 'paste' && (
            <textarea
              ...
              placeholder="Dán toàn bộ nội dung văn bản vào đây..."
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {renderInputMode('1. Tài liệu CV của bạn', cvMode, setCvMode, cvDocId, setCvDocId, cvText, setCvText)}
      {renderInputMode('2. Job Description (JD)', jdMode, setJdMode, jdDocId, setJdDocId, jdText, setJdText)}
    </div>
  );
}
