...
const PasswordField = ({ label, icon: Icon, value, onChange, placeholder, id }: PasswordFieldProps) => {
...
const ProfilePage = () => {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = (error as any)?.error || (error as any)?.message || "Cập nhật thất bại, thử lại sau";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const msg = (error as any)?.error || (error as any)?.message || "Đổi mật khẩu thất bại, thử lại sau";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
