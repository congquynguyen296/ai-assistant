...
const QuizResultPage = () => {
  ...
  interface QuizResultsPayload {
    ...
    completedAt: string;
    answersSubmitted: Record<number, string>;
  }

  const location = useLocation();
  const { results: stateResults, quizId: stateQuizId } = location.state as { results?: QuizResultsPayload, quizId: string } || {};
  const quizId = paramQuizId || stateQuizId;

  const [resultData, setResultData] = useState<any>(null);
  const [localResults, setLocalResults] = useState<any>(stateResults || null);
  ...
  useEffect(() => {
    ...
    if (quizId && !stateResults) {
      fetchResults();
    }
  }, [quizId, stateResults]);

  if (loading) return <LoadingSpinner />;
  if (!resultData && !localResults) return null;

  const data = resultData || { quiz: { title: "Quiz", score: localResults?.score }, results: [] };
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-8 mb-8">
          <div className="p-4 bg-slate-50 rounded-lg border border-slate-100 min-w-[140px]">
            ...
            <p className="text-xl font-bold text-emerald-600">
              {results.filter((r: any) => r.isCorrect).length}{" "}
